...
class UniformDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = (torch.randint(
            high=255, size=self.size).float() - 127.5) / 5418.75
        return sample
...
def getTrainData(dataset='imagenet',
    ...
    if dataset == 'imagenet':
        ...
        partial_train_dataset, _ = torch.utils.data.random_split(train_dataset, [dataset_length, len(
            train_dataset)-dataset_length], generator=torch.Generator().manual_seed(0))
